...
def find_similar_movies(name_of_the_movie):
    User_movie_rating = moviemat[name_of_the_movie]
    list_name = moviemat.columns.values
    if(mode == 1):
        print("***************************** ", name_of_the_movie," rating Data ******************************************")
        print(User_movie_rating.head())
        print("******************************************************************************************", "\n")
    # similar = moviemat.corrwith(User_movie_rating)

    # similar = moviemat.corr()
    # similar = similar[name_of_the_movie]
    lis = []
    final_dict = dict()

    for x in range(len(moviemat.columns)):
        corr_value = moviemat[moviemat.columns[x]].corr(User_movie_rating)
...
def find_similar_movies_cosine_sim(name_of_the_movie):
    ...
    for x in range(len(list_name)):
        b = moviemat[moviemat.columns[x]].values
        a = moviemat[name_of_the_movie]
        # print(a[0])
        for i in range(1,500): # limiting user Rating to 100 users
        ...
        for i  in range(500):# limiting user Rating to 100 users
            if (isinstance(b[i], np.float64) and np.isnan(b[i])==False):
                dic_1[list_name[i]] = b[i]
